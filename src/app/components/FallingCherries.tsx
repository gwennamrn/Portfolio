...
export const FallingCherries = memo(function FallingCherries() {
  const cherries = useMemo(generateCherries, []);

  const keyframes = useMemo(() => {
    return cherries.map((c) => `
      @keyframes fall-${c.id} {
        0% {
          transform: translateY(-60px) translateX(0px) rotate(${c.rotation}deg);
        }
        25% {
          transform: translateY(25vh) translateX(${c.swayAmount}px) rotate(${c.rotation + 90}deg);
        }
        50% {
          transform: translateY(50vh) translateX(-${c.swayAmount * 0.6}px) rotate(${c.rotation + 180}deg);
        }
        75% {
          transform: translateY(75vh) translateX(${c.swayAmount * 0.4}px) rotate(${c.rotation + 270}deg);
        }
        100% {
          transform: translateY(105vh) translateX(-${c.swayAmount * 0.2}px) rotate(${c.rotation + 360}deg);
        }
      }
    `).join('\n');
  }, [cherries]);
